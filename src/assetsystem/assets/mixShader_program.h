...
namespace mix
{
    namespace assetsystem
    {
        class mixShader_program : public mix::assetsystem::mixAsset_item
        {
            ...
            mixShader_program (const mix::platform::mixAsset_path& path, shader* vertex_shader, shader* fragment_shader);
            ...
            void compile (shader* vertex_shader, shader* fragment_shader);
            void use () const;

          
            void set_1f (const std::string& s, float f);
            void set_1i (const std::string& s, int i);
