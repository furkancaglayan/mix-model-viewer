...
namespace mix
{
    namespace containers
    {
        ...
        const mix::assetsystem::mixAsset_item* tree_node::search_with_guid (const mix::core::mixGuid& key) const
        {
            return search_with_member<const mix::core::mixGuid&> (key, &mix::assetsystem::mixAsset_item::get_guid);
        }

        const mix::assetsystem::mixAsset_item* tree_node::search_with_base_name (const std::string& name) const
        {
            return search_with_member<const std::string&> (name, &mix::assetsystem::mixAsset_item::get_base_name);
        }

          const mix::assetsystem::mixAsset_item* tree_node::search_with_full_name (const std::string& name) const
        {
